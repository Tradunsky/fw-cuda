...
*
*******************************************/
__global__ void fwStepK(int k, double weightMatrix[], int matrixWidth, int verticeCount) {
	/* This thread’s matrix column */
	int col = blockIdx.x * blockDim.x + threadIdx.x;
	if (col < verticeCount) {
		int currentEdge = matrixWidth * blockIdx.y + col;
		__shared__ double trkc;
		/* this row, kth column */
		...
		if (trkc != INT_MAX) {
			/* this column, kth row */
			double tckr = weightMatrix[k * matrixWidth + col];
			if (tckr != INT_MAX) {
				double intermediateWeight = trkc + tckr;
...
//util
bool matrixIsEquals(double* matrixA, double* matrixB, int length, int width) {
	bool isEquals = true;
...
bool readGraph(string filePath, Graph* graph) {
	bool isGraphRead = true;
	...
	if (fileExtension == "csv") {
		...
		isGraphRead = graph->readFromCsvFile(fileName);
	}
	else if (fileExtension == "txt") {
		//		INFO("Reading graph from txt file: %s", fileName);
		isGraphRead = graph->readFromTextFile(fileName);
	}
	else {
		//it's csv graph
...
string FWarshall::csvFwGpu(string filePath) {
	Graph graph;
	graph.addCsvGraph(filePath);
	//	DEBUG("Original graph csv for GPU: %s", graph.toCsv().c_str());
	double* hostWeightMatrix = graph.toWeightMatrix();
	double* deviceWeightMatrix;
	// Number of vertices
	...
	int weightMatrixSize = matrixWidth * verticeCount * sizeof(double);
	// Copy the array to newly-allocated global memory
	CUDA_CHECK_RETURN(cudaMalloc(&deviceWeightMatrix, weightMatrixSize));
	CUDA_CHECK_RETURN(cudaMemcpy(deviceWeightMatrix, hostWeightMatrix, weightMatrixSize,cudaMemcpyHostToDevice));
...
wstring FWarshall::csvWFwGpu(string filePath) {
	Graph graph;
	if (readGraph(filePath, &graph)){
		//	DEBUG("Original graph csv for GPU: %s", graph.toCsv().c_str());
		double* hostWeightMatrix = graph.toWeightMatrix();
		double* deviceWeightMatrix;
		// Number of vertices
		...
		// the kth vertex as an intermediate
		for (int k = 0; k < verticeCount; k++) {
			fwStepK << <blocksPerGrid, threadsPerBlock >> >(k, deviceWeightMatrix, matrixWidth, verticeCount);
			CUDA_CHECK_RETURN(cudaThreadSynchronize());
		}
		//TODO: copy result to new pointer.
		//Result will copy to graph weight matrix...
		...
		wstring csvWieghtMatrix = graph.toWWeightMatrixCsv();
		CUDA_CHECK_RETURN(cudaFree(deviceWeightMatrix));
		return csvWieghtMatrix;
	}
	return L"";
...
string FWarshall::uniFwGpu(string filePath) {
	Graph graph;
	if (readGraph(filePath, &graph)){
		//	DEBUG("Original graph csv for GPU: %s", graph.toCsv().c_str());
		double* hostWeightMatrix = graph.toWeightMatrix();
...
string FWarshall::csvFwCpu(string filePath) {
	Graph graph;
	if (readGraph(filePath, &graph)){
		//	DEBUG("Original graph csv for CPU: %s", graph.toCsv().c_str());
		int verticeCount = graph.getVerticiesCount();
		int weightMatrixWidth = graph.getWeightMatrixWidth();
		double* weightMatrix = graph.toWeightMatrix();
		for (int k = 0; k < verticeCount; k++) {
			for (int i = 0; i < verticeCount; i++) {
				for (int j = 0; j < verticeCount; j++) {
					double kc = weightMatrix[i * weightMatrixWidth + k];
					if (kc != INT_MAX) {
						double kr = weightMatrix[k * weightMatrixWidth + j];
						if (kr != INT_MAX) {
							double intermediateWeight = kc + kr;
