...
bool Graph::readGraphByLine(const char* argv) {
	...
	ifstream graphFile;
	string dummyString;
	double thisWeight; /* weight of the edge just read from file */
...
double* Graph::refreshWeightMatrix() {
	//	N = nameToNum.size();
...
void Graph::addEdge(string vName1, string vName2, double weight) {
...
void Graph::addCsvGraph(string line) {
	...
	while (getline(linestream, vname1, CSV_SEPARATOR)) {
		getline(linestream, vname2, CSV_SEPARATOR);
		getline(linestream, weight, CSV_SEPARATOR);
		double thisWeight = atof(weight.c_str());
		//assign map of edges...
		addEdge(vname1, vname2, thisWeight);
	}
}
...
void Graph::printWeightMatrix() {
	...
	for (i = nameToNum.begin(); i != nameToNum.end(); ++i) {
		if (i->second < 10) {
			...
			for (j = nameToNum.begin(); j != nameToNum.end(); ++j) {
				if (j->second < 10) {
					double distance = weightMatrix[i->second * Na + j->second];
					if (distance != INT_MAX)
						printf("%d\t", distance);
					else
						printf("--\t");
...
string Graph::toCsv() {
	...
	for (i = weightMap.begin(); i != weightMap.end(); ++i) {
		for (j = i->second.begin(); j != i->second.end(); ++j) {
			sstm << i->first << CSV_SEPARATOR << j->first << CSV_SEPARATOR << j->second << CSV_SEPARATOR;
		}
	}
	return sstm.str();
}
...
string Graph::toWeightMatrixCsv() {
	...
	for (i = nameToNum.begin(); i != nameToNum.end(); ++i) {
		for (j = nameToNum.begin(); j != nameToNum.end(); ++j) {
			double distance = weightMatrix[i->second * Na + j->second];
			sstm << i->first << CSV_SEPARATOR << j->first << CSV_SEPARATOR;
			if (distance != INT_MAX){
				sstm << distance;
			}
			else{
...
wstring Graph::toWWeightMatrixCsv() {
	map<string, int>::iterator i, j, prevLast = --nameToNum.end();
	wstring csvWeightMatrix;
	wstringstream sstm;	
	for (i = nameToNum.begin(); i != nameToNum.end(); ++i) {
		for (j = nameToNum.begin(); j != nameToNum.end(); ++j) {
			double distance = weightMatrix[i->second * Na + j->second];
			sstm << i->first.c_str() << CSV_SEPARATOR << j->first.c_str() << CSV_SEPARATOR;
			if (distance != INT_MAX){
				sstm << distance;
			}
			else{
